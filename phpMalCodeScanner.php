...
// Set to your email:
define('SEND_EMAIL_ALERTS_TO','nyonis@mytum.de');
...
class phpMalCodeScan {
	...
	function scan($dir) {
		...
		if(!is_array($files)) {
			echo 'Unable to scan directory ' . $dir . '.  Please make sure proper permissions have been set.<br>';
	...
	function sendalert() {
		if(count($this->infected_files) != 0) {
			echo 'MALICIOUS CODE FOUND!<br>';
			echo 'The following files appear to be infected: <br>';
